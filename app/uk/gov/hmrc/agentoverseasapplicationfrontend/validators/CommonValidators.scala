...
object CommonValidators {
  ...
  def firstName: Mapping[String] = text verifying commonFormConstraint("firstName", NameRegex, NameMaxLength)

  def lastName: Mapping[String] = text verifying commonFormConstraint("lastName", NameRegex, NameMaxLength)
  ...
  private def agentCodeConstraint(
    ...
    maxLength: Int = AgentCodeMaxLength): Constraint[String] = Constraint[String] { fieldValue: String =>
  ...
  private def amlsBodyConstraint(bodies: Set[String]): Constraint[String] = Constraint[String] { fieldValue: String =>
    Constraints.nonEmpty(fieldValue) match {
      case _: Invalid => Invalid(ValidationError("error.moneyLaunderingCompliance.amlsbody.empty"))
