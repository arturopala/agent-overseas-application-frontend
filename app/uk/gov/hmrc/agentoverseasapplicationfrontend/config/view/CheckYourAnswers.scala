...
package uk.gov.hmrc.agentoverseasapplicationfrontend.config.view
...
import uk.gov.hmrc.agentoverseasapplicationfrontend.models.{AgentSession, No, Yes, YesNo}
...
object AnswerRow {
  def apply(id: String, question: String, answerLines: Seq[String], changeLink: Option[Call] = None)(
    implicit messages: Messages): AnswerRow =
    AnswerRow(id, question, answerLines, changeLink, Some(Messages("checkAnswers.change.button")))
}
...
object CheckYourAnswers {
  ...
  private def formatFileName(fileName: String): String =
    if (fileName.length > 20) s"${fileName.take(10)}...${fileName.takeRight(10)}" else fileName
